...
def compute_y_displacement(ds, threshold_z=-1000):
    y = ds['y'][:, :]
    z = ds['z'][:, :]
    ...
    y_final = np.array([y[threshold_indices[n], n] for n in range(n_particles)])
    y_initial = y[0, :]
    return y_final - y_initial
...
y_displacement = compute_y_displacement(ds)
#transit_times_in_rp = transit_times / rotation_period_Europa

plt.hist(y_displacement, bins=50, density=True)
plt.xlabel("delta y (m)")
plt.ylabel("Probability density")
#plt.savefig("delta_y_probability_distribution.png", dpi=300)

delta_y400 = compute_y_displacement_with_y_bounds(ds, -400000, -200000)
plt.hist(delta_y400 / 1000, bins=20)

delta_y50 = compute_y_displacement_with_y_bounds(ds, -50000, 50000)
plt.hist(delta_y50 / 1000, bins=20)
